...
:disabled="loading">
    ...
    <div class="page-body">
      <form @submit.prevent="handleSubmit" class="contest-form-container">
...
required
        ...
        <div class="mb-3">
          ...
          <select id="allowedType"
class="form-control"
v-model="formData.allowed_submission_type"
              :disabled="loading">
...
:disabled="loading" />
            ...
            <!-- Multi-Parameter Scoring Content -->
...
:disabled="loading" />
        ...
        <!-- MediaWiki category URLs for article validation (Optional) -->
        <div class="mb-3">
          <label class="form-label">
            Category URLs
...
:disabled="loading" />
        ...
        <div class="mb-3">
          ...
          <input type="url"
class="form-control"
id="outreachDashboardUrl"
v-model="formData.outreach_dashboard_url"
...
:disabled="loading">
          ...
          <button type="submit" class="btn btn-primary" :disabled="loading">
            ...
            <span v-else><i class="fas fa-plus me-2"></i>Create Contest</span>
          </button>
        </div>
      </form>
    </div>
  ...
  setup() {
    ...
    onMounted(async () => {
      ...
      if (currentUser.value) {
        console.log('[CREATE] User loaded:', currentUser.value.username)
      } else {
        console.warn('[CREATE] User not loaded after retries')
      }
    })
    ...
    watch(currentUser, (newUser, oldUser) => {
      if (newUser) {
        console.log('[CREATE] User loaded/changed:', {
...
/* Modal header styles (now page header) */
.page-header {
  background-color: var(--wiki-primary);
  color: white;
  border-bottom: none;
...
/* Modal body styles (now page body) */
.page-body {
  flex: 1;
  overflow-y: auto;
  padding: 2rem;
...
/* Modal footer styles (now page footer) */
.page-footer {
  border-top: 1px solid var(--wiki-border);
  padding: 1rem 1.5rem;
...
[data-theme="dark"] .organizer-placeholder-text {
...
[data-theme="dark"] .jury-selection-box {
