<template>
  <!-- Full-screen modal for requesting contest creation -->
  <div class="modal fade" id="requestContestModal" tabindex="-1">
    <div class="modal-dialog modal-fullscreen">
      <div class="modal-content">
        <!-- Modal header with Wikipedia primary color -->
        <div class="modal-header">
          <h5 class="modal-title">Request to Create Contest</h5>
  ...
  setup(props, { emit }) {
    ...
    const handleSubmit = async () => {
      ...
      try {
        ...
        const result = await store.requestContest(contestData)
        if (result.success) {
          ...
          showAlert(result.error || 'Failed to submit contest request', 'danger')
        }
      } catch (error) {
        showAlert('Failed to submit contest request: ' + error.message, 'danger')
      } finally {
        loading.value = false
      }
    }
